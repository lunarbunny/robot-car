...
const char* INFRARED_returnChar(){
    ...
    for (int i = 0; finalString[i] != '\0'; ++i) {
        ...
        if (finalString[i] == '*') {tempCountStar ++;}
        if (tempCountStar == 2) {
...
void INFRARED_decodeCharTree() {
    ...
    if (isBackwards) {
        ...
        if (varcharStartEndStar[0]){ // Currently using varcharStartEndStar, 
            ...
            {
                // U1AK = KA1U
                if (compareArray(varcharStartEndStar, letterU)==0) {varCharASCII = 'K';}
                else if (compareArray(varcharStartEndStar, no1)==0) {varCharASCII = 'A';}
                else if (compareArray(varcharStartEndStar, letterA)==0) {varCharASCII = '1';}
                else {varCharASCII = 'U';}
            } 
            ...
            {
                // W3CM = QG7-
                if (compareArray(varcharStartEndStar, letterW)==0) {varCharASCII = 'Q';}
                else if (compareArray(varcharStartEndStar, no3)==0) {varCharASCII = 'G';}
                else if (compareArray(varcharStartEndStar, letterC)==0) {varCharASCII = '7';}
                else {varCharASCII = '-';}
            }
            else if (varcharStartEndStar[6])
            {
                //.8HR = LB2V
                if (compareArray(varcharStartEndStar, charPeriod)==0) {varCharASCII = 'L';}
                else if (compareArray(varcharStartEndStar, no8)==0) {varCharASCII = 'B';}
                else if (compareArray(varcharStartEndStar, letterH)==0) {varCharASCII = '2';}
                else {varCharASCII = 'V';}
            }
            else 
            {
                // OE5Y = X4DN
                if (compareArray(varcharStartEndStar, letterO)==0) {varCharASCII = 'X';}
                else if (compareArray(varcharStartEndStar, letterE)==0) {varCharASCII = '4';}
                else if (compareArray(varcharStartEndStar, no5)==0) {varCharASCII = 'D';}
                else {varCharASCII = 'N';}
            }
        } 
        ...
        // QGNTDJ7%40LSBIPF296-+X*/$VZ SPACE
            ...
            if (varcharStartEndStar[8]) // Ending number 1
            {
                if (varcharStartEndStar[2]) {
                    //V2BL = RH8.
                    if (compareArray(varcharStartEndStar, letterV)==0) {varCharASCII = 'R';}
                    else if (compareArray(varcharStartEndStar, no2)==0) {varCharASCII = 'H';}
                    else if (compareArray(varcharStartEndStar, letterB)==0) {varCharASCII = '8';}
                    else {varCharASCII = '.';}
                }
                else if (varcharStartEndStar[6]) {
                    //-7GQ = MC3W
                    if (compareArray(varcharStartEndStar, charDash)==0) {varCharASCII = 'M';}
                    else if (compareArray(varcharStartEndStar, no7)==0) {varCharASCII = 'C';}
                    else if (compareArray(varcharStartEndStar, letterG)==0) {varCharASCII = '3';}
                    else {varCharASCII = 'W';}
                }
                else {
                    //X4DN = OE5Y
                    if (compareArray(varcharStartEndStar, letterX)==0) {varCharASCII = 'O';}
                    else if (compareArray(varcharStartEndStar, no4)==0) {varCharASCII = 'E';}
                    else if (compareArray(varcharStartEndStar, letterD)==0) {varCharASCII = '5';}
                    else {varCharASCII = 'Y';}
                }
            ...
            else // Ending number 0
            {
                ...
                if (varcharStartEndStar[1]) 
                {
                    ...
                    if (varcharStartEndStar[2]) {
                        // Z SPACE
                        if (compareArray(varcharStartEndStar, letterZ)==0) {varCharASCII = 'T';}
                        else {varCharASCII = 'S';}
                    }
                    else {
                        // $/*+
                        if (compareArray(varcharStartEndStar, charStartEndStar)==0) {varCharASCII = 'P';}
                ...
                else if (varcharStartEndStar[2])
                {
                    ...
                    if (varcharStartEndStar[4]){
                        //6FP = J0*
                        if (compareArray(varcharStartEndStar, no6)==0) {varCharASCII = 'J';}
                        else if (compareArray(varcharStartEndStar, letterF)==0) {varCharASCII = '0';}
                        else {varCharASCII = '*';}
                    }
                    else {
                        //9IS = I9 SPACE
                        if (compareArray(varcharStartEndStar, no9)==0) {varCharASCII = 'I';}
                        else if (compareArray(varcharStartEndStar, letterI)==0) {varCharASCII = '9';}
                        else {varCharASCII = ' ';}
                    }
                }
                else {
                    // 0%JT = F$6Z
                    if (compareArray(varcharStartEndStar, no0)==0) {varCharASCII = 'F';}
                    else if (compareArray(varcharStartEndStar, charPercentage)==0) {varCharASCII = '$';}
                    else if (compareArray(varcharStartEndStar, letterJ)==0) {varCharASCII = '6';}
                    else {varCharASCII = 'Z';}
                }
            }
        }
